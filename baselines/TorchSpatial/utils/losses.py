import torch
import TorchSpatial.utils.utils as ut
import math
...
def embedding_loss(model, params, loc_feat, loc_class, user_ids, inds, neg_rand_type='spherical'):
    ...
    # Convert loc_feat from (lon,lat) to (lat, lon)
    #loc_feat = loc_feat[:, [1, 0]]

    loc_cat = torch.cat((loc_feat, loc_feat_rand), 0)
    #loc_cat = loc_cat.cpu().detach().numpy()
    ...
    out_dim, in_dim = model.class_emb.weight.shape
    # print(f"class_emb: out_dim: {out_dim}, in_dim: {in_dim}, or {out_dim} by {in_dim} matrix")
    # print(f"loc_emb: {loc_emb.shape}")
